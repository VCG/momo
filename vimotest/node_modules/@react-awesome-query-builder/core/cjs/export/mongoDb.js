...
var formatGroup = function formatGroup(parents, item, config, meta) {
  ...
  var groupOperatorDef = groupOperator && (0, _configUtils.getOperatorConfig)(config, groupOperator, groupField) || null;
  ...
  var reversedGroupOpDef = (0, _configUtils.getOperatorConfig)(config, reversedGroupOp, groupField);
  ...
  var list = children.map(function (currentChild) {
    return formatItem([].concat((0, _toConsumableArray2["default"])(parents), [item]), currentChild, config, meta, revChildren, mode != "array", mode == "array" ? function (f) {
  ...
  if (groupField) {
    if (mode == "array") {
      ...
      resultQuery = formatItem(parents, item.set("type", "rule"), config, meta, filterNot, false, function (_f) {
        return filterQuery;
      }, totalQuery);
      resultQuery = {
        "$expr": resultQuery
...
var formatRule = function formatRule(parents, item, config, meta) {
  ...
  var args = [formattedField, operator, _value !== undefined && formattedValue == null ? _value : formattedValue, useExpr, valueSrcs.length > 1 ? valueSrcs : valueSrcs[0], valueTypes.length > 1 ? valueTypes : valueTypes[0], (0, _stuff.omit)(operatorDefinition, _stuff.opDefKeysToOmit), operatorOptions, fieldDef];
